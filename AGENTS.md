...
- 2026-05-18: NO panel/card/box framing anywhere on the site — this includes the top bar (`.bar`), footer (`.foot`), homepage project listing (`.listing` / `.row`), sidebars, log items, or any new component. Specifically: do not combine `border: 1px solid var(--rule)` with `background: var(--surface)` on a container. Use at most a single dashed/dotted hairline divider (`border-top`/`border-bottom`) or nothing. Hover states use color shifts and a leading `$` glyph, not a tinted background block. Content sits on `--bg` directly.
